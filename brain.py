...
def parse_intent(text: str, team_names: list[str]) -> dict:
    ...
    prompt = (
        ...
        "ГЛАВНОЕ ПРАВИЛО: если одна команда обыгрывает/побеждает/проходит другую ИЛИ "
        "одна команда проигрывает/уступает/вылетает от другой — это ВСЕГДА intent=match "
        "(а не scenario), даже если стадия не названа.\n"
        "intent:\n"
        "• \"match\" — исход одного матча двух команд. winner — кто прошёл дальше, loser — "
        "кто вылетел. «X обыграет Y» → winner=X, loser=Y. «X проиграет Y» → winner=Y, loser=X.\n"
